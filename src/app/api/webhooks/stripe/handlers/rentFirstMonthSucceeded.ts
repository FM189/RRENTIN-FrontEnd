...
function buildTransactions(txBase: TxBase, params: {
  ...
  const { bookingId, tenantId, ownerId, rentalAmount, platformFeeRate,
    platformFee, vatOnPlatformFee, stripeFee, ownerNet,
    ownerContractFee, tenantContractFee, tenantContractFeeVat } = params;
  ...
  const txs: TxEntry[] = [
    ...
    { ...txBase, type: "owner_payout", userId: ownerId,  amount: ownerNet,         description: `Owner payout (first month) — booking ${bookingId}` },
  ];
...
function buildLineItems(params: {
  ...
  const { rentalAmount, tenantContractFee, tenantContractFeeVat,
...
export async function handleRentFirstMonthSucceeded(pi: Stripe.PaymentIntent): Promise<void> {
  ...
  const transactions = buildTransactions(txBase, {
    ...
    stripeFee, ownerNet, ownerContractFee, tenantContractFee, tenantContractFeeVat,
  });

  const lineItems = buildLineItems({
    rentalAmount, tenantContractFee, tenantContractFeeVat,
