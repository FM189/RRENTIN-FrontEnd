...
export async function handleRentOverdueManualSucceeded(pi: Stripe.PaymentIntent): Promise<void> {
  const bookingId   = pi.metadata?.bookingId;
  const monthNumber = Number(pi.metadata?.monthNumber ?? 0);
  const hasLateFee  = pi.metadata?.hasLateFee === "true";
  if (!bookingId) return;

  const booking = await RentBooking.findById(bookingId);
  if (!booking || booking.status !== "active") return;
  if (!booking.overdueAmount || booking.overdueAmount <= 0) return;

  const overdueAmount = pi.amount / 100;
  ...
  const lateFeeRate      = fees?.lateFeeRate      ?? 0.15;
  ...
  const monthLabel = `Month ${monthNumber} (overdue manual)`;

  const transactions: TxEntry[] = [
    { ...txBase, type: "rent_payment", userId: booking.tenantId as Types.ObjectId, amount: baseRent,         description: `${monthLabel} rent — booking ${bookingId}` },
  ...
  if (hasLateFee && lateFee > 0) {
    transactions.push({
      ...txBase,
      type:        "late_fee",
      userId:      null,
      amount:      lateFee,
      description: `Late fee (${(lateFeeRate * 100).toFixed(0)}% of THB ${baseRent.toLocaleString()}) — booking ${bookingId}`,
    });
  }
  ...
  const lineItems: { description: string; amount: number; vatRate: number; vatAmount: number; total: number }[] = [
    { description: `${monthLabel} Rent`, amount: baseRent, vatRate: 0, vatAmount: 0, total: baseRent },
  ];

  if (hasLateFee && lateFee > 0) {
    lineItems.push({
      description: `Late Fee (${(lateFeeRate * 100).toFixed(0)}% of THB ${baseRent.toLocaleString()})`,
      amount:      lateFee,
      vatRate:     0,
      vatAmount:   0,
      total:       lateFee,
  ...
  await Promise.all([
    ...
    Invoice.create({
      ...
      status:    "issued",
      issuedAt:  new Date(),
    }).then((invoice) => generateAndStoreInvoicePdf(invoice)),
  ]);
}
