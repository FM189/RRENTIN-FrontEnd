...
export default function TenantVisitRequestsClient({
  ...
  const navigate = (newPage: number, newStatus: string) => {
    ...
    router.push(`/dashboard/tenant/visit-requests${params.toString() ? `?${params}` : ""}`);
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      {requests.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {requests.map((req) => (
            <VisitRequestCard key={req.id} request={req} onRefresh={handleRefresh} />
          ))}
        </div>
      )}
